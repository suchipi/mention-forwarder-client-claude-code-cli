...
**A session that opens as a copy of another thread's is told so, for the one run that copies it.** `forkParent` is true exactly then, and it is what puts the line about a shared working directory in the system prompt: the thread it was copied from is still live, with a `claude` of its own in the same `cwd`, and nothing here keeps the two apart. A later restart of the same thread is not opening a copy, so it does not say it again.
