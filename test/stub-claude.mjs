...
if (argvFile !== undefined) writeFileSync(argvFile, JSON.stringify(process.argv.slice(2)));

const resumed = process.argv.slice(2).find((arg) => arg.startsWith("--resume="));
const sessionId = resumed === undefined ? "11111111-2222-3333-4444-555555555555" : resumed.slice("--resume=".length);
...
function emitInit() {
  out({ type: "system", subtype: "init", session_id: sessionId, model: "stub", permissionMode: "default", tools: ["Write"] });
}

function emitText(text) {
  out({
    type: "assistant",
    message: { role: "assistant", id: "msg_stub", content: [{ type: "text", text }] },
...
function emitResult(text, extra = {}) {
  out({ type: "result", subtype: "success", is_error: false, result: text, permission_denials: [], session_id: sessionId, ...extra });
}
...
function runTurn(text) {
  ...
  if ((scenario === "ask" || scenario === "question") && turns === 1) {
    ...
    out({
      type: "assistant",
      message: { role: "assistant", id: "msg_stub_tool", content: [{ type: "tool_use", id: "toolu_1", name: "Write", input: { file_path: "notes.txt", content: "hi" } }] },
      parent_tool_use_id: null,
      session_id: sessionId,
    });
    out({
      ...
      request: {
        ...
        input:
          scenario === "question"
            ? { questions: [{ question: "Tabs or spaces?", header: "Style", options: [{ label: "Tabs", description: "hard tabs" }, { label: "Spaces", description: "soft tabs" }] }] }
...
function settleAsk(response) {
  ...
  out({
    type: "user",
    message: { role: "user", content: [{ type: "tool_result", tool_use_id: "toolu_1", is_error: !allowed, content: allowed ? "written" : String(response?.message ?? "") }] },
    parent_tool_use_id: null,
    session_id: sessionId,
  });
  const text = allowed ? "stub wrote the file" : `stub was told: ${String(response?.message ?? "")}`;
  emitText(text);
  emitResult(text, allowed ? {} : { permission_denials: [{ tool_name: "Write", tool_use_id: "toolu_1", tool_input: { file_path: "notes.txt" } }] });
}
...
createInterface({ input: process.stdin }).on("line", (line) => {
  ...
  if (frame.type === "control_response") {
    if (frame.response?.request_id === pendingAsk) settleAsk(frame.response.response);
    return;
  }
  if (frame.type === "user") {
    const text = frame.message.content.map((block) => block.text ?? "").join("");
    runTurn(text);
  }
});
